...
For the $l$-layer of the graph convolution, the node representations that were output from the previous layer $l-1$ are used as input.
...
A nonlinear activation function is also applied at each layer to introduce non-linearity to the model {KOrderConnectivity}.
