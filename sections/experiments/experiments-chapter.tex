...
\\\\
To test whether it is actually the context and side-information that increase the performance of our model in a non-context specific setting, we run our proposed method, CSGCN-ADJ, where we use different inputs for the datasets YelpNC and ML1M, and measure the performances with NDCG@20.
First of all, our method is run with both side-information and context. 
Then we remove side-information, remove context and finally, we remove both context and side-information.
This experiment is conducted to test the effects of context and side-information on our model so that we can try to answer RQ3.
\begin{figure}
    \begin{tikzpicture}
        \begin{axis}[
            xlabel=Epoch,
...
In \Cref{fig:ablation_study_1}, we see the performance on NDCG@20 of our model with different input.
We see that the more input the model takes, the longer it takes to converge to its best performance.
...
To conclude from the study on the YelpNC dataset, the usage of context is not able to provide better recommendations in a non-context specific setting.
Side-information, however, performs the best on this metric on this dataset, which suggest that side-information in our model is able to slightly improve results, compared to having no extra input.
...
\end{figure}
On \Cref{fig:ablation_study_2} the NDGC@20 results of ML1M is shown for our model with different types of input. 
For this dataset, the model where neither the context or side-information is used in the input, is actually the best performing version.
...
Since both \Cref{fig:ablation_study_1,fig:ablation_study_2} show that the model is worse with context included, we can conclude that context does not increase the performance of our model in a non-context specific setting.
