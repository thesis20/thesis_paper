...
To answer whether side-information improves the results for context-aware recommendations, we performed an ablation study on the YelpNC dataset.
...
\Cref{subfig:ablation_study_context_specific_1} shows the HR@20 for CSGCN-ADJ on YelpNC with different combinations of input. 
We see that when side-information is included in the input for CSGCN-ADJ it does not change the performance much compared to excluding it.
...
\\\\
\Cref{subfig:ablation_study_context_specific_2} shows HR@20 for CSGCN-IS on YelpNC with different inputs.
...
However, the context seems to have a lot larger effect on the performance of the models, as would be expected for context-aware recommendations.
Additionally, it may be worth noting that the context used for these experiments is mainly based on discretized values based on timestamps, which may or may not be a useful contextual dimension.
