...
\end{itemize}
NGCF is a model proposed by Xiang Wang et al. as a way to utilize the GCN model proposed in \cite{KOrderConnectivity} for recommendation purposes.
It makes use of all three parts of a GCN described in \Cref{sec:preliminaries}, and combines it with utilizing BPR loss to generate negative samples.
...
Both the CSGCN models and LightGCN extend the codebase of NGCF, so they are natural to be included in the comparison to show that adding side-information contextual information hopefully improves the accuracy of these methods.\\
...
\multirow{4}{*}{Yelp-NC} & HR@20  & 0.0075  & 0.0142  & 0.0070 & 0.0545 & 0.0761  &  \\
...
\end{figure*}
\Cref{fig:ablation_study_1} shows the performance on NDCG@20 of CSGCN-ADJ with different input.
...
Since both \Cref{fig:ablation_study_1,fig:ablation_study_2} show that the model is worse with context included, it can be concluded that context does not increase the performance of CSGCN-ADJ in a non-context specific setting.
