...
To facilitate the model architecture, each node in the graph is represented by a low-dimensional also called an embedding.
...
This differs from \textit{CSGCN-IS} since we now model context in a similar fashion to side-information, where each context value has a single embedding instead of having a embedding for each context value and item pair.
