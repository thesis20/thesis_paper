...
\end{equation}
In \Cref{eq:csgcn_is_gc_layer_item,eq:csgcn_is_gc_layer_user} the GCN layer operation is shown for respectively item and user embeddings, where $\mathcal{N}_{u}$ are the neighboring nodes for user $u$, including both items and user side-information, and $\mathcal{N}_{i}$ are the neighboring nodes for item $i$. 
A neighbor-node is denoted as $un$ or $in$ for respectively user neighbors and item neighbors, and represents either a user or item side-information node for an $in$ node, and either an item or user side-information node for a $un$ node.
\\\\
The convolution layer updates user and item nodes' representations by summing the normalized representations of the neighbor nodes.
Only the item and user nodes are updated as shown, as these embeddings are the only ones used beyond the convolution layer in the prediction function, further described in \Cref{subsec:csgcn_is_score_prediction}.
The side-info nodes' representations are also updated in the layer but are not being output from the layer since their representations are integrated into the item and user node representations.
...
\\\\
The model parameters are updated by optimizing the BPR objective function in \Cref{eq:loss-func}.
...
Additionally, we add the identity matrix $I$ to the last quadrant such that the context embeddings are only propagated with information about themselves in each convolution.
...
We found that for this version of the model, it gave slightly better performance compared to multipluying with $D^{\frac{1}{2}}$ on both sides of the adjacency matrix.
The final embedding matrix after $l$ layers is obtained in the same way as CSGCN-IS where we take the mean of the nodes' representation at each layer.
\\\\
