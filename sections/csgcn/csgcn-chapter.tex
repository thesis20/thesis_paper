...
The graph can be represented a an adjacency matrix, so the input adjacency matrix is constructed by inserting a 1 for each user, item and side-information that are connected, in their corresponding cell.
Each unique user, item, side-information are represented by an embedding.
...
The adjacency representation of the input graph facilitates the convolution operation by multiplying the adjacency matrix with the embeddings.
Each multiplication produces a layer output with embeddings propagated with neighborhood information.
These layer outputs are then combined together with the initial embeddings to produce a set of final embedding which are used in the prediction function.
...
\end{figure}
This quadripartite graph can be represented as an adjacency matrix $A \in \mathbb{R}^{|U|\times|I|\times|US| \times |IS|}$.
