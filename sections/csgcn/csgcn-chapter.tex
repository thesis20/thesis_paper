...
For a specific model parameter, this is done by calculating the gradient of the loss function at each iteration with respect to the model parameter, and then updating it according to the gradient using a learning rate.
The gradient is a vector of partial derivates, which defines the slope of the function.
The parameters can then by updated in the negative direction of the function based on the learning rate, in order to approach a local minima. 
