...
Instead, they were proposed in the recent years of neural networks where deep learning has gained high popularity, and as such it was build directly on top of a complex model.
To investigate whether this was useful, they present SGCN, a simplified version of GCNs that could have been preceeded GCNs if the traditional path had been taken.
This lead them to remove the linear transformation and nonlinear activation, to reach a simplified linear model.
...
Based on these considerations, CSGCN-IS is based on these observations made in the SGCN \cite{SimplifyingGCN} and LightGCN models \cite{LightGCN}, which has applied these considerations to their model and proven that it outperforms regular GCNs, which used a traditional GCN model with all three components \cite{LightGCN}.
