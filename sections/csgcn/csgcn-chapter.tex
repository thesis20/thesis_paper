...
\end{figure}
The model (\Cref{fig:csgcn_is_model}) itself is very similar to LightGCN.
...
These new nodes are illustrated as dark nodes in the figure.
\\
...
As mentioned, neighbor nodes for user are item nodes and user side-information nodes, and for items the neighbors are user nodes and item side-information nodes
For each additional layer, information is passed from nodes that are further away from a central node. 
